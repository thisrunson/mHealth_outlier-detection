...
# Directories
#####
top_dir <- "/Users/bderenzi/CSEDropbox/Dropbox/Outlier Detection/Organizations/CARE - Bihar/"
output_dir <- paste(top_dir, "naive/output_data/", sep="")
input_file <- paste(top_dir, "bihar_cleaned_15JUL2013-ASHAs_only.csv", sep="")


load_data <- function() {
	CHW_file <- read.csv(input_file, sep=",", header=T, as.is=T, na.strings=c("","---"))
...
run_mma <- function(data, agg_col, cat_cols) {
	...
	for( col in cat_cols ) {
		# print("run: ")
		col_vals <- unique(data[,col][!is.na(data[,col])])
		...
		for (chw in unique_CHWs) {
			...
			for(col_val in col_vals) {
				# fill it in
				frequencies[i,"chw"] <- chw
				mine <- sub[sub[col] == col_val,]
				mine <- mine[!is.na(mine)]
				frequencies[i,paste(col,"_",col_val, sep="")] <- length(mine)
				i <- i + 1
			}
		}
...
sort_and_plot <- function(scores, window_end) {
	#sort outliers by score, highest to lowest
	sorted_scores <- scores[order(-scores$score),]
	
	#print(window_end)
	
	output_csv <- paste(output_dir, "sorted_scores_", "WEEKS", window_end, ".csv", sep="")

	write.table(sorted_scores, output_csv, row.names=F)

	plot(sorted_scores$score, main=window_end, ylab="score", xlab="CHWs - all questions")
}
...
#TODO: change the file name to reflect weeks/months/whatever
pdf(paste(output_dir, "sorted_scores_sliding_window_by_week.pdf", sep=""), height=16, width=8, onefile=TRUE)

#uncomment to test only a couple cols
#cols <- c("form.bp2.not_yet_maternal_emergency", "form.bp1.counsel_ifa")

#TODO: change the "by = week" to months to get month data
date_breaks <- seq.Date(from=as.Date('2012-07-31'), to=as.Date('2013-08-01'), by="week") 

i <- 1

#produce a sliding window to move across data
for(date in date_breaks){
	if(i != 4){
		i <- i + 1
		next
	}
	print ( "going")
	sliding_window <- subset(CHW_file, CHW_file$date_number > date_breaks[i] & CHW_file$date_number <= date_breaks[i+1])
	...
	# date_label <- as.Date(date)
	date_label <- date_breaks[i]
	print(date_label)

	output_file <- paste(output_dir, "scores_questions-weeks/scores_output-questions_by_week-", date_label, ".csv", sep="")
	write.table(scores, output_file, row.names=F)
	
	##throws an error when encounters missing values; skip these rows if so
    #possibleError <- tryCatch(
	#	summed_scores <- aggregate(score ~ chw, data = scores, FUN = sum),
    #  	error=function(e) e
	#	)
	#	 
	...
	#sort_and_plot(summed_scores, date_breaks[i])

	#increment the window by a time unit
	i <- i + 1
}

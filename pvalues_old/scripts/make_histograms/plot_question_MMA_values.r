...
input_file <- paste(output_dir, "scores_output-all.csv", sep="")
...
plot_questions <- function(one_question){
	...
	#plot data
	plot <- ggplot(data=one_question, aes(score), environment = environment()) + geom_histogram(binwidth=0.01) + ggtitle(title) +
			geom_vline(aes(xintercept = mean_score), colour="blue") + geom_vline(aes(xintercept = one_sd), colour="red") + 
			geom_vline(aes(xintercept = two_sd), color="red", linetype="longdash") + coord_cartesian(xlim = c(0, 10), ylim = c(0, 100)) 
...
pdf(output_pdf <- paste(output_dir, "question_MMA_values-xlim=20,ylim=100", ".pdf", sep=""), height=12, width=12, onefile=TRUE)

...
input_file <- paste(output_dir, "find_outliers/over_per_month.csv", sep="")
...
plot_df <- function(over_df){
	...
	pdf(output_pdf <- paste(output_dir, "find_outliers/over_per_month-ratios", ".pdf", sep=""), height=12, width=12, onefile=TRUE)
	...
	plot <- ggplot(data=over_df, aes(over_ratio), environment = environment()) + geom_histogram(binwidth=0.01) + 
			ggtitle("Percent Counts - % of Months Each CHW is over Threshold - 'cumulative' weeks data") + 

...
plot_df <- function(percentover_df){
	...
	# 		ggtitle("Percent Counts - % of Questions per CHW over Threshold - 'cumulative' months data") + 
	...
	plot <- ggplot(data=percentover_df, aes(over_count), environment = environment()) + geom_histogram(binwidth=.5) + 
			ggtitle("Score Counts - Number of Questions per CHW over Threshold - 'cumulative' months data") + geom_vline(aes(xintercept = ratio_mean), colour="blue") + 

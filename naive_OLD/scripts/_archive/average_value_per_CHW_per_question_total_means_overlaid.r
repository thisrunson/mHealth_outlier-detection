...
output_dir <- "/Users/bderenzi/CSEDropbox/Dropbox/Outlier Detection/graphs_output/avg_val_with_CHW/"
...
plot_everything <- function(all_data_df, min, max, plot_method){	
	...
	return(ggplot(data=all_data_df, aes(date), environment = environment()) + geom_line(aes(y=whole_mean, colour="whole_mean")) + geom_line(aes(y=individual_mean, colour="individual_mean")) + geom_ribbon(error_line, alpha=0.5) + ylim(min, max) + ggtitle(plot_method))
}
...
ted_process <- function(CHW_file, CHW_subset, col, method) {
    ...
    return(plot_everything(all_data_df, min, max, method))
}
...
for(col in cols ) {
	...
	for(CHW in unique_CHWs){
		...
		pdf(paste(output_dir, col, "/", col, CHW, ".pdf", sep=""), height=16, width=8, onefile=TRUE)
		
		p1 <- ted_process(CHW_file, CHW_subset, col, "days")
		p2 <- ted_process(CHW_file, CHW_subset, col, "weeks")
		p3 <- ted_process(CHW_file, CHW_subset, col, "months")

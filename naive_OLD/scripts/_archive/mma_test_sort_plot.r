...
input_file <- "/Users/tedmccarthy/Documents/Research\ Projects/CHW\ Outlier\ Detection/modified_data/ASHAs_only.csv"
...
sort_and_plot <- function(scores, window_end) {
	...
	output_csv <- paste(output_dir, "sorted_scores_", "WEEKS_with_INFS", window_end, ".csv", sep="")
...
pdf(paste(output_dir, "sorted_scores_sliding_window_by_week_WITH_INFs", ".pdf", sep=""), height=16, width=8, onefile=TRUE)
...
date_breaks <- seq.Date(from=as.Date('2012-07-31'), to=as.Date('2013-06-01'), by="week") 
...
for(date in date_breaks){
	...
	for (col_name in cols){
		...
		#test to see if the column contains empty string elements
		if("" %in% data[,col_name]){
	...
	#if("Inf" %in% scores$score){
	#	scores$score[scores$score == "Inf"] <- 0
	#}

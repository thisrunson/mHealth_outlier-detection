...
# Directories
#####
output_file <- "/Users/tedmccarthy/Documents/Research\ Projects/CHW\ Outlier\ Detection/modified_data/scores_output-all_collection.csv"
...
run_mma <- function(data, agg_col, cat_cols) {
	...
	for( col in cat_cols ) {
		...
		outlier_scores_for_col <- mma_compute_outlier_scores(frequencies, col_vals, unique_CHWs, col) #problem is here - (list) object cannot be coerced to type double
...
week_start <- "2012-07-31" 
week_end <- "2012-08-08"

month_start <- "2012-07-31" 
month_end <- "2012-09-01"

all_collection_start <- "2012-06-30"
all_collection_end <- "2013-05-13" 
...
for (col_name in cols){
	...
	#test to see if the column contains any empty string elements
	if("" %in% data[,col_name]){

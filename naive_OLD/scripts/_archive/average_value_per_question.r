...
process_data <- function(unique_dates, this_question, question, method) {
	...
	df <- data.frame(date=rep(NA,N), mean=rep(NA,N), stringsAsFactors=FALSE)
	...
	for ( i in seq_along(days) ) {
		...
		df[i,] <- c(toString(days[i]), now_mean)
		previous_day <- days[i]
	}
	####
	# clean
	...
	return(ggplot(data=df, aes(date, mean)) + geom_line() + ylim(min,max) + ggtitle(method))
}

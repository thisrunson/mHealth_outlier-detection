...
# Directories
#####
output_dir <- "/Users/tedmccarthy/Dropbox/Outlier\ Detection/normalized_data/output_data/find_outliers/"

input_aggregate <- paste(output_dir, "top_outliers-questions_months_over.csv", sep="")
input_total <- paste(output_dir, "top_outliers-total_questions_over.csv", sep="")

plot_outliers <- function(outlier_df){
	
	pdf(output_pdf <- paste(output_dir, "aggregate_vs_total-outlier_scores-months", ".pdf", sep=""), height=12, width=12, onefile=TRUE)

	#plot a histogram of the CHW count scores
	plot <- ggplot(outlier_df, aes(x=aggregate_count, y=total_count)) + geom_point() + ggtitle("Question/month outliers vs. total outliers - Counts") +
			xlab("Aggregate Count") + ylab("Total Count") #+ geom_abline(slope=correlation)

	print(plot)	
	
	#plot a histogram of the CHW ratio scores
	plot <- ggplot(outlier_df, aes(x=aggregate_percent, y=total_percent)) + geom_point() + ggtitle("Question/month outliers vs. total outliers - Percents") +
...
outliers_df<- data.frame(chw = character(0), aggregate_count = numeric(0), aggregate_percent = numeric(0), 
				chw2 = character(0), total_count = numeric(0), total_percent = numeric(0))

unique_chws <- unique(aggregate_df$c)

#shared is the intersection of CHWs in the two df's - make sure they're the same length
shared <- intersect(aggregate_df$c, total_df$c)
...
for (c in unique_chws){
	...
	#make sure this chw is in both the months/weeks and total data, else skip to next chw
	if (!(c %in% shared)) next
	...
	print(one_row)
	colnames(one_row) <- c("chw", "aggregate_count", "aggregate_percent", "agg_months", "chw2", "total_count", "total_percent", "tot_weeks")
	
	print(one_row)
	one_row <- subset(one_row, select=c(chw, aggregate_count, aggregate_percent, chw2, total_count, total_percent))
	
	print(one_row)
	outliers_df <- rbind(outliers_df, one_row)
}

colnames(outliers_df) <- c("chw", "aggregate_count", "aggregate_percent", "chw2", "total_count", "total_percent")
...
output_csv <- paste(output_dir, "aggregate_vs_total-months.csv", sep="")

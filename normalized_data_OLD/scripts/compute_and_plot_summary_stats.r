...
plot_means <- function(data){
	...
	return(ggplot(data=data, aes(data[,window]), environment = environment()) + geom_point(aes(y=mean, colour = mean)) + 
			geom_errorbar(error_line, alpha=0.5, colour="red") + ylim(min, max) + ggtitle(paste("Scores - means and SDs - per ", window)) + 
...
pdf(output_pdf <- paste(output_dir, "means_SDs-", window, ".pdf", sep=""))

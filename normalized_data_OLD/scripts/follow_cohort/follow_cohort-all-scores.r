...
plot_rank <- function(one_chw, num_weeks){
	...
	plot <- ggplot(data=one_chw, aes(x=one_chw[,window]), environment = environment()) + way_to_print + geom_line(aes(y=one_chw$week_mean)) +
				geom_ribbon(error_ribbon) + xlab(window) + ylab("score") + coord_cartesian(xlim = c(xmin, xmax), ylim = c(0, 8)) +
...
pdf(output_pdf <- paste(output_dir, "top_outliers-all-scores", "_N=", n, "_", method, "_", window, ".pdf", sep=""), height=12, width=12, onefile=TRUE)

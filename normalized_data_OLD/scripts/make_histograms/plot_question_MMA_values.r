...
plot_questions <- function(one_question){
	...
	geom_vline(aes(xintercept = two_sd), color="red", linetype="longdash") #+ coord_cartesian(xlim = c(0, 1.5), ylim = c(0, 250)) 
...
pdf(output_pdf <- paste(output_dir, "question_MMA_values", ".pdf", sep=""), height=12, width=12, onefile=TRUE)

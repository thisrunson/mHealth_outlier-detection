...
plot_questions <- function(one_question){
	...
	#plot data
	plot <- ggplot(data=one_question, aes(score), environment = environment()) + geom_histogram(binwidth=0.002) + ggtitle(title) +
	geom_vline(aes(xintercept = mean_score), colour="blue") + geom_vline(aes(xintercept = one_sd), colour="red") + 
	geom_vline(aes(xintercept = two_sd), color="red", linetype="longdash") + coord_cartesian(xlim = c(0, .2), ylim = c(0, 2500)) 
...
pdf(output_pdf <- paste(output_dir, "MMA_values-score-week-question-bin=.002,xlim=.2,ylim=2500", ".pdf", sep=""), height=12, width=12, onefile=TRUE)
